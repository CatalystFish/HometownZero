...
8. **v0.5 (Aug 23 night):** area-POI ways (shop/amenity/craft/office/tourism/leisure/healthcare closed rings without a building tag) join via shoelace centroid; hotel→`residential`, museum/cinema/theatre→`civic`, sports facilities→`weapons_outdoors`. Building universe grew 219→234 on Pike Place (shop polygons now count as structures themselves).
9. **v0.6 (Aug 24 night):** area-POIs whose centroid falls inside a known building footprint are deduped away (same structure mapped twice). Verified: 5 deduped / 2 kept on Pike Place. Multi-city comparison generated: Portland 314 bldgs (66% classified, 11/13 categories — strongest launch candidate), Austin 437 bldgs (31% classified — weak POI mapping). Samples: `Scripts/samples/{pike_place,portland,austin}.json`.
